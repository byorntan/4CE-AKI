...
# description: String of diagnosis (for easier readability)
comorbid_ref <- read.csv("comorbid.csv")
...
demographics <- demographics %>% select(patient_id,siteid,admission_date,days_since_admission,last_discharge_date,still_in_hospital,severe_date,severe,death_date,deceased,sex,age_group,race,race_collected)
# course <- course[,c(8,1,3:7)]
observations <- observations %>% select(patient_id,siteid,days_since_admission,concept_type,concept_code,value)
